...
class User {
 ...
 factory User.fromJson(Map<String, dynamic> json) {
  ...
  List<UserProduct> userProductsList = json['userProducts'] != null 
    ? (json['userProducts'] as List).map((i) => UserProduct.fromJson(i)).toList()
    : []; // Empty list if null

  return User(
    id: json['id'] ?? 0, // Default to 0 if id is null
    userName: json['userName'] ?? '', // Default to an empty string if userName is null
    password: json['password'] ?? '',
    phoneNumber: json['phoneNumber'] ?? '',
    latitude: (json['latitude'] as num?)?.toDouble() ?? 0.0, // Default to 0.0 if latitude is null
    longitude: (json['longitude'] as num?)?.toDouble() ?? 0.0, // Default to 0.0 if longitude is null
