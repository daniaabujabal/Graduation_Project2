...
class ApiService {
  final String baseUrl = "https://localhost:44387/api/Pharmacy";
  ...
  Future<List<dynamic>> searchProductsAndPharmacies(String query,
    ...
    if (response.statusCode == 200) {
      ...
      searchResults.addAll(matchedPharmacies.map((pharmacy) {
        return {'type': 'pharmacy', 'data': pharmacy};
      }));
      ...
      for (var pharmacyJson in jsonResponse['pharmacies']) {
        ...
        searchResults.addAll(matchedProducts.map((product) {
          return {'type': 'product', 'data': product, 'pharmacy': pharmacy};
        }));
      }
      ...
      if (sortBy != null) {
        if (sortBy == 'price') {
          searchResults.sort(
              (a, b) => a['data'].publicPrice.compareTo(b['data'].publicPrice));
